...
<%
	ShopVO vo = dao.getDetail(Integer.parseInt(request.getParameter("custno")));
%>
<!DOCTYPE html>
<html>
<head>
...
<%@ include file="../pages/nav.jsp" %>
	<section>
		<h3>홈쇼핑 회원 정보 수정</h3>
		<form action="update_impl.jsp">
			<table class="big">
				<tr>
					<th id="big">회원번호(수정불가)</th>
					<td><input type="text" name="custno" size="20" value="<%=vo.getCustno() %>" readonly></td>
				</tr>
				<tr>
					<th>회원성명</th>
					<td><input type="text" name="custname" size="20" required="required" value="<%=vo.getCustname() %>"></td>
				</tr>
				<tr>
					<th>회원전화</th>
				...
				<tr>
					<th colspan="2"><input type="submit" value="수정" class="btn" onclick="return joinCheck()"> &nbsp; <input type="button" value="조회" class="btn" onclick="location.href='list.jsp'"></th>
				</tr>
			</table>
		</form>
	</section>
